...
void enforce_minimum_distance0(std::vector<double>& knot_vector, float min_distance) {
    if (knot_vector.empty()) return;

    for (size_t i = 1; i < knot_vector.size(); ++i) {
        if (i >= knot_vector.size() || (i - 1) < 0) {
            // out of bounds
            return;
        }
        if (knot_vector[i] - knot_vector[i - 1] <= min_distance) {
            knot_vector[i] = knot_vector[i - 1] + min_distance;
        }
    }
}
...
void enforce_minimum_distance(std::vector<double>& knot_vector) {

    if (knot_vector.empty()) return;

    std::vector<double> unique_knots;
    unique_knots.push_back(knot_vector[0]);
	
    for (size_t i = 1; i < knot_vector.size(); ++i) {

        // Check if this knot is not equal to the last one added
        if (knot_vector[i] != unique_knots.back()) {
            unique_knots.push_back(knot_vector[i]);
        }
    }

    knot_vector = unique_knots;
    std::cout << "Enforce minimun distance TEST" << std::endl;
    print_vector(knot_vector);
}
// hacer una función que no saque los duplicados
...
void Bsurface::generate_interpolation_knot_vectors( int degree1, int degree2,
	...
	// initialize U
	Utemp = update_knot_vector_based_on_grid(degree1, degree2, true, Ugrid, Vgrid, grid_map, per, Utemp);  //TEST
    	//enforce_minimum_distance(Utemp);  // Adjust the 1e-4 value as needed
	...
	// initialize V
	Vtemp = update_knot_vector_based_on_grid(degree1, degree2, false, Ugrid, Vgrid, grid_map, per, Vtemp); //TEST
    	//enforce_minimum_distance(Vtemp);  // Adjust the 1e-4 value as needed
	...
	Vknot = Vtemp;
	// TEST
	//enforce_minimum_distance(Uknot);
	//enforce_minimum_distance(Vknot);
	
	//print_vector(Uknot);
	//print_vector(Vknot);
	
	enforce_minimum_distance(Uknot); 
	Uknot = update_knot_vector_based_on_grid(degree1, degree2, true, Ugrid, Vgrid, grid_map, per, Uknot);
	
	enforce_minimum_distance(Vknot); 
	Vknot = update_knot_vector_based_on_grid(degree1, degree2, true, Ugrid, Vgrid, grid_map, per, Vknot);
	
	//enforce_minimum_distance0(Uknot,0.1); 
	//enforce_minimum_distance0(Vknot,0.1); 
